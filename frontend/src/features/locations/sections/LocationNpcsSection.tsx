...
export function LocationNpcsSection({ campaignId, location, isGm, enabled, partyEnabled }: Props) {
  ...
  const picker = useLinkedEntityList<NPC, NPC>({
    linked: npcsHere,
    candidates: allNpcs ?? [],
  ...
  return (
    ...
    >
      <section className="space-y-4">
        ...
        {npcsHere.length === 0 && !picker.pickerOpen ? (
                  ...
                  <div className="flex items-stretch">
                    ...
                    {isGm && (picker.isAskingRemove(npc.id) ? (
                      <InlineConfirm
                        ...
                        onYes={() => { handleRemoveNpc(npc); picker.cancelRemove(); }}
                        onNo={picker.cancelRemove}
                        yesDisabled={saveNpc.isPending}
                      />
                    ) : (
                      <button
                        onClick={() => picker.askRemove(npc.id)}

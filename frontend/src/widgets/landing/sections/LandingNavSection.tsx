...
export function LandingNavSection() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-md border-b border-outline-variant/10 px-10 py-5">
      {/* Relative wrapper so the nav links can be absolutely centered
          against the bar. Width changes in the right-side cluster
          (language toggle labels) no longer shift the center anchors. */}
      <div className="relative flex items-center">
        ...
        <div className="hidden md:flex items-center gap-8 absolute left-1/2 -translate-x-1/2 pointer-events-none">
          <a href="#features" className="pointer-events-auto text-xs font-label uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors">{t('nav.features')}</a>
          <a href="#roadmap" className="pointer-events-auto text-xs font-label uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors">{t('nav.roadmap')}</a>
          <Link to={`/${currentLang}/changelog`} className="pointer-events-auto text-xs font-label uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors">{t('nav.changelog')}</Link>
        </div>

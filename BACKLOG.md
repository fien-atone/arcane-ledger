...
| F-26 | 🟡 | Campaign system selector. Add `Campaign.system: String?` to the schema. On campaign create/edit drawer, show a `<Select>` with a fixed list of known systems ("D&D 5e 2014", "D&D 5e 2024", "Pathfinder 2e", "Call of Cthulhu 7e", "Savage Worlds", "Generic / Homebrew") plus a "Custom…" option. Selecting "Custom" reveals a free-text input for the user's own system name. Stored as a plain string, no validation beyond max length. Read-only display on CampaignsPage card + dashboard header. Later enables system-specific presets (F-24 extension) and system-specific UI affordances like character sheet export (F-8). Depends on: nothing. | — |
